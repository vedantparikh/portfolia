...
class TradeRepublicParser(PDFParser):
    ...
    def _extract_transactions(self, full_text: str) -> Tuple[List[ParsedTransaction], List[str]]:
        ...
        for i, line in enumerate(processed_lines):
            ...
            if type_str == 'Handel':
                trade_match = re.search(r"(?i)(buy trade|sell trade|savings plan execution)\s+([A-Z0-9]{12})\s+(.*?),\s+quantity:\s*([\d\.,]+)", desc)
                if trade_match:
                ...
                parsed_transaction.update({"transaction_type": "dividend", "name": f"Dividend for {symbol}", "symbol": symbol})
