...
const ParsedDataTable = ({ parsedData, onSave, onCancel, portfolios = [] }) => {
  ...
  const handleSymbolChange = (value, index) => {
    ...
    if (!value) {
      ...
      handleFieldChange(index, "asset_id", null);
    }
  };
  ...
  const handleBulkSave = async () => {
    ...
    if (transactionsWithoutAssetId.length > 0) {
      toast.error(
        `Error: ${transactionsWithoutAssetId.length} transaction(s) are missing a matched asset. Please edit each required row and select an asset.`
      );
      return;
    }
      ...
      <div className="bg-gray-800 rounded-lg shadow-xl max-w-7xl w-full max-h-[90vh] overflow-hidden">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-300">
                    {editingRow === index ? (
                      <input
                        ...
                        onChange={(e) =>
                          handleFieldChange(index, "fees", e.target.value)
                        }
                        className="input-field py-1 px-2 text-sm w-full"
                      />
                    ) : (
        ...
        <div className="flex items-center justify-between p-6 border-t border-gray-700">
          <div className="text-sm text-gray-400">
            ...
            {filteredTransactions.length !== 1 ? "s" : ""} ready to import
          </div>
          <div className="flex items-center space-x-3">
            <button
              onClick={onCancel}
