...
import { assetAPI, watchlistAPI } from "../../services/api";
...
const Watchlist = () => {
  ...
  useEffect(() => {
    ...
    if (selectedWatchlist && autoRefreshEnabled) {
      console.log(
        `[Watchlist] Starting auto-refresh every ${refreshInterval}ms for watchlist ${selectedWatchlist.id}`
      );
  ...
  const refreshSelectedWatchlist = async () => {
    ...
    try {
      console.log(
        `[Watchlist] Refreshing watchlist ${selectedWatchlist.id} with real-time data`
      );
      setIsRefreshing(true);

      // Fetch the latest data with real-time prices
      const updatedWatchlist = await watchlistAPI.getWatchlist(
  ...
  const handleUpdateWatchlist = async (watchlistId, updateData) => {
    try {
      ...
      const updatedWatchlist = await watchlistAPI.updateWatchlist(
        watchlistId,
        updateData
      );
      setWatchlists((prev) =>
  ...
  const handleAddSymbol = async (symbolData) => {
    ...
    try {
      await assetAPI.createAsset({
        ...
        name:
          symbolData.long_name || symbolData.short_name || "Unknown Company",
        asset_type: symbolData.asset_type || "EQUITY",
      ...
      symbolData.company_name =
        symbolData.long_name ||
        symbolData.short_name ||
        symbolData.longname ||
        symbolData.shortname ||
        symbolData.name;

      // Pass the complete symbol data to the API
      const newItem = await watchlistAPI.addItemToWatchlist(
        selectedWatchlist.id,
        symbolData
      );
      ...
      if (error.response?.status === 400) {
        errorMessage =
          ...
          "Invalid symbol or symbol already exists";
  ...
  const handleReorderItems = async (itemIds) => {
    ...
    try {
      ...
      const reorderedItems = itemIds
        .map((id) => selectedWatchlist.items.find((item) => item.id === id))
        .filter(Boolean);
  ...
  if (loading) {
    return (
      <div className="min-h-screen gradient-bg flex items-center justify-center">
        <LoadingSpinner size="lg" text="Loading watchlists..." centered />
      </div>
    );
  }
                ...
                <>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      {selectedWatchlist?.items?.length || 0} symbols •{" "}
                      {selectedWatchlist?.is_public ? "Public" : "Private"}
                    </p>
                  </div>
                </>
              ) : (
              ...
              {viewMode === "grid" && (
                <>
                  ...
                  >
                    <Plus size={16} />
                    <span>
                      {creatingWatchlist ? "Creating..." : "New Watchlist"}
                    </span>
                  </button>
                </>
              )}
                ...
                {loading ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 mb-6">
                      Create your first watchlist to start tracking market
                      symbols
                    </p>
                    <button
                      onClick={() => setShowCreateModal(true)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-400 text-center">
                        Start tracking your favorite symbols
                      </p>
                    </div>
                  </div>
                )}
